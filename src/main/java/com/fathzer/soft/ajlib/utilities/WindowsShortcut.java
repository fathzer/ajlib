 ...
 * Also available at https://github.com/codebling/WindowsShortcuts
 * 
...
public class WindowsShortcut {
    ...
    private void parseLink(final byte[] link) throws ParseException {
        try {
            ...
            final int file_start = 0x4c + shellLen;
            ...
            int nextStringStart = file_start + file_location_size;
            ...
            if ((flags & has_description) > 0) {
                final int string_len = bytesToWord(link, nextStringStart) * 2; // times 2 because UTF-16
                description = getUTF16String(link, nextStringStart + 2, string_len);
                nextStringStart = nextStringStart + string_len + 2;
            }

            // if relative path is present, parse it
            if ((flags & has_relative_path) > 0) {
                final int string_len = bytesToWord(link, nextStringStart) * 2; // times 2 because UTF-16
                this.relativePath = getUTF16String(link, nextStringStart + 2, string_len);
                nextStringStart = nextStringStart + string_len + 2;
            }
            ...
            if ((flags & has_working_directory) > 0) {
                final int string_len = bytesToWord(link, nextStringStart) * 2; // times 2 because UTF-16
                this.workingDirectory = getUTF16String(link, nextStringStart + 2, string_len);
                nextStringStart = nextStringStart + string_len + 2;
            }
            ...
            if ((flags & has_command_line_arguments) > 0) {
                final int string_len = bytesToWord(link, nextStringStart) * 2; // times 2 because UTF-16
                this.commandLineArguments = getUTF16String(link, nextStringStart + 2, string_len);
            }
